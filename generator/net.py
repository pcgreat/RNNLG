...
class Model(object):
    ...
    def saveNet(self):
        ...
        self.updateNumpyParams()
        print("GG")
        for name in self.learn_vars:
            print(name)
            print(eval(name))

        bundle = {'learn': {}, 'data': {}, 'gen': {}, 'model': {}, 'mode': {}, 'params': {}}
        for name in self.learn_vars:
            bundle['learn'][name] = eval(name)
        # bundle['learn'] = dict( [(name,eval(name)) for name in self.learn_vars]  )
        for name in self.data_vars:
            bundle['data'][name] = eval(name)
        # bundle['data'] = dict( [(name,eval(name)) for name in self.data_vars]   )
        for name in self.gen_vars:
            bundle['gen'][name] = eval(name)
        # bundle['gen'] = dict( [(name,eval(name)) for name in self.gen_vars]    )
        for name in self.model_vars:
            bundle['model'][name] = eval(name)
        # bundle['model'] = dict( [(name,eval(name)) for name in self.model_vars]  )
        for name in self.mode_vars:
            bundle['mode'][name] = eval(name)
        # bundle['mode'] = dict( [(name,eval(name)) for name in self.mode_vars]   )
        for name in self.params_vars:
            bundle['params'][name] = eval(name)
        # bundle['params'] = dict( [(name,eval(name)) for name in self.params_vars] )
